    ...
    transform(input:any[], property:string, type:string = 'asc'): any{
		...
		return input.sort( (a,b)=>{
			if( props.length == 1 ) {
				...
				let aa = a[props[0]][props[1]], bb = b[props[0]][props[1]];
			} else if( props.length == 3 ) {
				let aa = a[props[0]][props[1]][props[2]], bb = b[props[0]][props[1]][props[2]];
			} else if( props.length == 4 ) {
				let aa = a[props[0]][props[1]][props[2]][props[3]], bb = b[props[0]][props[1]][props[2]][props[3]];
			} else if( props.length == 5 ) {
				let aa = a[props[0]][props[1]][props[2]][props[3]][props[4]], bb = b[props[0]][props[1]][props[2]][props[4]];
			}
			
			if( !aa ) aa = '';
			if( !bb ) bb = '';
			if((isNaN(parseFloat(aa)) || !isFinite(aa)) || (isNaN(parseFloat(bb)) || !isFinite(bb))) {
				if(aa.toLowerCase() < bb.toLowerCase()) return -1*sign;
				if(aa.toLowerCase() > bb.toLowerCase()) return 1*sign;
			}
			else {
